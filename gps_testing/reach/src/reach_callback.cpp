  ...
  for(int i=0;i<(int)msg->data.size();i++){
    ...
    switch(decode_nmea){
      case 0: // scan NMEA data for UBLOX data 
        if((uint8_t)msg->data.at(i) == 181){ decode_nmea = 1; } break;
      case 1: // potential UBLOX data found
        if((uint8_t)msg->data.at(i) == 98){ decode_nmea = 2; }
        else if((uint8_t)msg->data.at(i) == 181){ decode_nmea = 1; }
        else{ decode_nmea = 0; }
        break;
      case 2: // check for nav data
        if((uint8_t)msg->data.at(i) == 1){ decode_nmea = 3; }else{ decode_nmea = 0; } break;
      case 3: // check for llh data
        if((uint8_t)msg->data.at(i) == 2){ decode_nmea = 4; }else{ decode_nmea = 0; } break;
      case 4: // check proper length
        if((uint8_t)msg->data.at(i) == 28){ count = 1; decode_nmea = 4; }
        else if((uint8_t)msg->data.at(i) == 0){
      ...
      case 5: // parse llh data
        switch(decode_llh){
          case 0: // time of week data
            ...
            gps_rtk.time += (uint8_t)msg->data.at(i) << count*8; break;
          case 1: // longitude
            ROS_INFO("count: %d, longitude: %f",count,gps_rtk.longitude);
            gps_rtk.longitude += (255 - (uint8_t)msg->data.at(i)) << count*8; break;
          case 2: // latitude
            ROS_INFO("count: %d, latitude: %f",count,gps_rtk.latitude);
            gps_rtk.latitude += (uint8_t)msg->data.at(i) << count*8; break;
          case 3: // altitude -- ellipsoidal
            ROS_INFO("count: %d, altitude: %f",count,gps_rtk.altitude);
            gps_rtk.altitude += (uint8_t)msg->data.at(i) << count*8; break;
          case 4: // altitude -- mean sea level
            ROS_INFO("count: %d, amsl: %f",count,gps_rtk.amsl);
            gps_rtk.amsl += (uint8_t)msg->data.at(i) << count*8; break;
          case 5: // horizontal accuracy
            gps_rtk.hacc += (uint8_t)msg->data.at(i) << count*8; break;
          case 6: // vertical accuracy
            gps_rtk.vacc += (uint8_t)msg->data.at(i) << count*8; break;
