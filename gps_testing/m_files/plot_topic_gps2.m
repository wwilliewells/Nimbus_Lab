...
%     axis([min(fail(2,:))-0.1 max(fail(2,:))+0.1 min(fail(1,:))-0.1 max(fail(1,:))+0.1])
    ...
    %% subject gps
    try import_bag=bagReader(sprintf('../../bags/%s_bag%i.bag',...
        vehicle_type,test), '/w/subject_gps');
        ...
        try import_bag=bagReader(sprintf('../../bags/%s_bag%i.bag',...
            vehicle_type,test), '/a/subject_gps');
    ...
    %% subject pose
    try import_bag=bagReader(sprintf('../../bags/%s_bag%i.bag',vehicle_type,...
        test), '/w/subject_pose');
      sp_ns=table2array(import_bag(:,5));
        ...
        try import_bag=bagReader(sprintf('../../bags/%s_bag%i.bag',...
            vehicle_type,test), '/a/subject_pose');
...
%         madt=1;
%         devicetx=zeros(length(device_tx),50);
%         for i=1:1:length(device_tx)
%           w=cell2mat(device_tx(i))';
% %           n=1;
%           for k = 4:1:length(w)-3
%             if length(w)-6 > madt
%               madt = length(w)-6;
%             end
%             devicetx(i,k-3)=bitand(w(k),4095);
% %             n=n+1;
%           end    
%         end
%         devicetx=devicetx(1:i,1:madt);
%         %else
%             %device_tx=table2array(import_csv(:,1));
%         %end
%     else
...
%     z=z*1.2*2.54;
    ...
    if view_plot == 1
        ...
        axis([min(gx)-0.1 max(gx)+0.1 min(gy)-0.1 max(gy)+0.1])
        ...
        figure
        plot3(gx,gy,gps_estimate(:,3),'r.') % elipsoidal
        hold on 
        plot3(gx,gy,gps_estimate(:,4),'b.') % above mean sea level
        %plot3(x(mp),y(mp),z(mp),'k<')
        grid on
        grid minor
        axis([min(gx)-0.1 max(gx)+0.1 min(gy)-0.1 max(gy)+0.1 min(gps_estimate(:,3))-0.1 max(gps_estimate(:,4))+0.1])
        ...
        hold on 
        %plot3(gx,gy,gps_estimate(:,4),'b.') % task waypose
        %plot3(x(mp),y(mp),z(mp),'k<')
        grid on
        grid minor
    ...
    % 3-D position
        ...
        grid on
        grid minor
        axis([min(x)-0.1 max(x)+0.1 min(y)-0.1 max(y)+0.1 min(z)-0.1 max(z)+0.1])
