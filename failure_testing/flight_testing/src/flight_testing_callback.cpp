...
void FlightTest::callbackPose(const boost::shared_ptr<collab_msgs::SubjectPose const> &msg){
  ...
  // TODO: add pose error checks before assigning new task

  // set target position if not finished
  if(!publish_change && !change_pose && !last){
    ROS_INFO("manuever: %i",count);
    // set rotation -- TODO: change to switch - case
    ...
    // change position -- TODO: change to switch - case, check count uint8_t
  ...
  // confirm target with land target -- TODO: check if this block is still required
